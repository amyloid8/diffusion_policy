...
class PushObjectsRelativeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if action is not None:
            ...
            for i in range(n_steps):
                ...
                acceleration = self.k_p * (action) + self.k_v * (Vec2d(0, 0) - self.agent.velocity)
    ...
    def compute_reward(self, idx):
        goal_body = self._get_goal_pose_body(self.goal_poses[idx])
        goal_geom = pymunk_to_shapely(goal_body, self.blocks[idx].shapes)
        block_geom = pymunk_to_shapely(self.blocks[idx], self.blocks[idx].shapes)

        intersection_area = goal_geom.intersection(block_geom).area
        goal_area = goal_geom.area
        coverage = intersection_area / goal_area
        return np.clip(coverage / self.success_threshold, 0, 1), coverage > self.success_threshold

    def get_global_reward(self):
         # compute reward
        out = 0
        for i in range(len(self.goal_poses)):
            reward, done = self.compute_reward(i)
            out += reward
        return out / (len(self.goal_poses))
    
    def get_blocks_todo(self):
        out = []
        for i in range(len(self.goal_poses)):
            reward, done = self.compute_reward(i)
            if not done:
    ...
    def _get_obs(self):
        obs = np.array(
            tuple(self.blocks[self.active_idx].position - self.agent.position) \
            + (self.blocks[self.active_idx].angle % (2 * np.pi),) \
            + tuple(self.goal_poses[self.active_idx].position - self.agent.position) \
            + (self.goal_poses[self.active_idx].angle % (2 * np.pi),)) 
        return obs
    ...
    def _render_frame(self, mode, relative=False):
        ...
        # Draw goal pose.
        goal_body = self._get_goal_pose_body(self.goal_poses[self.active_idx] + offset)
    ...
    def _set_state(self, state):
        ...
        for i in range(self.n_blocks):
            pos_block = state[3 * i:2 + 3 * i]
            rot_block = state[2 + 3 * i]
            ...
            if self.legacy:
                ...
                self.blocks[i].position = pos_block
                self.blocks[i].angle = rot_block
            else:
                self.blocks[i].angle = rot_block
                self.blocks[i].position = pos_block
